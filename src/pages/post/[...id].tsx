...
const Post: React.FC<Props> = ({
  ...
  const ogImage = createOgImage({
    title,
    meta: [new Date(date).toLocaleString('en-US', options as any), ...tags].join('・'),
  });
  ...
  return (
    <>
      ...
      <DarkMode /> {/* You can move this into a more appropriate spot if needed */}
      
      <div className={styles.post}>
        <PostHeader title={title} date={date} desc={desc} tags={tags} />
        <PostContent markdown={markdown} prevPost={prevPost} nextPost={nextPost} />
        <Comment />
        <Footer />
      </div>
    </>
